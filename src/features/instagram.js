...
export async function handleInstagramDownloader(sock, from, url) {
  ...
  await sock.sendMessage(from, { text: '📥 Mengunduh video Instagram...' });

  const tempFile = `${__dirname}/tmp_ig.mp4`;
  ...
  try {
    ...
    console.error(err);
    await sock.sendMessage(from, { text: '❌ Gagal mengunduh video Instagram' });
  }
}